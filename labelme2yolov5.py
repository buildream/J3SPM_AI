...
class Labelme2YOLO(object):
    ...
    def _train_test_split(self, folders, json_names, val_size, test_size):
        # 폴더가 존재하며 각 train, val, test 폴더를 체크합니다.
        if len(folders) > 0 and 'train' in folders and 'val' in folders and 'test' in folders:
            ...
            # 전체 데이터에서 test 데이터를 분리
            remaining_idxs, test_idxs = train_test_split(range(len(json_names)), test_size=test_size)
            test_json_names = [json_names[i] for i in test_idxs]

            # 남은 데이터에서 train과 validation 데이터를 분리
            train_idxs, val_idxs = train_test_split(remaining_idxs, test_size=val_size)
    ...
    #             # label_id = self._label_id_map[ shape['label'] for shape in json_data["label"]]
    #             # segments = [shape["points"] for shape in json_data["shapes"] if shape["shape_type"] == "polygon"]
    #             # print(label_id+segments)    
    #             # yolo_obj_list = self._get_yolo_object_list(json_data, img_path)
    ...
    def convert(self, val_size, test_size=None):
        # 데이터셋 파일 준비
        ...
        # 개별 어노테이션 파일 변환 및 저장
        for file_set, set_name in [(train_files, 'train'), (val_files, 'val'), (test_files, 'test')]:
            for json_file in file_set:
                # JSON 어노테이션 열기
                with open(os.path.join(self._json_dir, json_file), 'r') as file:
                    data = json.load(file)
                # 이미지 크기 추출
                img = utils.img_b64_to_arr(data["imageData"])
                img_h, img_w = img.shape[:2]
                # 각 폴리곤과 레이블 추출
                segments = [(self._label_id_map[shape["label"]], shape["points"]) for shape in data["shapes"] if shape["shape_type"] == "polygon"]
                # 좌표 정규화
                normalized_segments = [
                    (label_id, [(x/img_w, y/img_h) for x, y in points]) for label_id, points in segments]
                # normalized_segments = [
                # (label_id, [(round(x/img_w, 3), round(y/img_h, 3)) for x, y in points]) for label_id, points in segments]
                # 저장 경로 구성
                label_dir_path = os.path.join(self._save_path_pfx, f'labels/{set_name}/')

                # 디렉토리 생성
                os.makedirs(label_dir_path, exist_ok=True)

                # 파일 저장
                txt_path = os.path.join(label_dir_path, f"{json_file.split('.')[0]}.txt")
                with open(txt_path, 'w+') as f:
                    for label_id, points in normalized_segments:
                        # 정규화된 좌표를 소수점 3자리로 저장하며, 띄어쓰기로 구분
                        coords = ' '.join([f"{p[0]} {p[1]}" for p in points])
                        f.write(f"{label_id} {coords}\n")

                # 이미지도 복사
                self._save_yolo_image(data, json_file, self._image_dir_path, set_name)
